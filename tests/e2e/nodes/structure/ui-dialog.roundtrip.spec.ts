 ...
 * reference picker. P259 (ADR 0038): the field is now the canonical `route`
 * (renamed from `routeId`); the fixture below deploys the LEGACY `routeId` (plus
 * `layoutId`) so this doubly proves back-compat load + migrate-on-save.
 *
 ...
 *
 * The migration (`withReferenceFieldMigration`) lifts the legacy `routeId` into
 * `route` on open and re-seeds the carrier, so after Done the node carries ONLY
 * `route`. Single `assertEditorRoundTrip` call — the canonical way per
...
function roundTripFlow(): NodeDef[] {
    ...
    // LEGACY routeId is pre-set to the first route (P259: proves the migration
    // path; the routes also stay legacy `layoutId`). modal/closable defaults.
    return [
...
test.describe("ui-dialog — route reference open→save round-trip", () => {
    ...
    test("route survives open→Done and a value-change round-trips (legacy routeId fixture)", async ({ page, request }) => {
