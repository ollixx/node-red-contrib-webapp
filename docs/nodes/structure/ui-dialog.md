...
| `requiresGroup` | „Gruppen" | Textfeld (kommaseparierte Gruppennamen) | optional | Deklarativer Authz-Guard ([ADR 0041](../../adr/0041-auth-model-idp-agnostic-identity-trusted-header-first.md) §4, [auth.md](../concepts/auth.md)). Werte: beliebige Gruppennamen, kommasepariert (tolerant geparst: getrimmt, Leereinträge verworfen). Default: leer. **Leer/absent ⇒ nur Authentifizierung nötig** (P261-Verhalten). **Gesetzt ⇒ der User braucht mindestens eine der Gruppen** (ANY-of gegen `user.groups`; kein Policy-DSL). Server-seitig erzwungen: der Dialog fehlt im Snapshot **vollständig** (weder Struktur noch Daten — auch wenn sein Open-State via `?dialog=<id>` oder eine `open`-Action erzwungen wird, der Renderer filtert ihn vor dem Rendern aus), und `/event` an seine Kinder (Mount-Kette bis zum Dialog) wird mit **403** und strukturiertem Fehler `server.auth.event-denied` abgelehnt. Ein an eine Route gekoppelter Dialog (`route` gesetzt) erbt zusätzlich deren `requiresGroup`-Guard für den Event-Dispatch. Ohne Identitätsquelle (`auth.mode: "none"`) erfüllt niemand einen gesetzten Guard. **„visibleIf ist UX, Guard ist Sicherheit"**: einen Öffnen-Button via `visible`/Reactive (`user`-Global) auszublenden ist Bedienkomfort — die Sicherheit ist ausschließlich dieser server-seitige Guard. |
