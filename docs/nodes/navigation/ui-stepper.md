...
| `activeStep` | „Active Step Path" | typedInput (Binding) | **ja** | Binding auf die `id` des aktuell aktiven Schritts. Der Stepper zeigt diesen Schritt und seinen Slot; alle anderen Schritte sind inaktiv. Bindbare Arten: `state`, `store`, `query`, `routeParam`, `literal` sowie Node-RED-Standard-Arten (`msg`, `flow`, `global`, `jsonata`, `env`). Wird ein ungültiger Wert geliefert, fällt die Komponente auf den ersten Schritt zurück. |
