...
| `label` | „Label" | typedInput (alle Binding-Arten) | optional | Beschriftung neben dem Switch-Element (z. B. „Benachrichtigungen"). Wenn leer, wird kein Label angezeigt. Unterstützt alle Binding-Arten: `literal`, `state`, `store`, `query`, `routeParam`, `msg`, `flow`, `global`, `jsonata`, `env`. |
| `labelOn` | „Label On" | typedInput (alle Binding-Arten) | optional | Text, der im Eingeschaltet-Zustand neben oder innerhalb des Switches angezeigt wird (z. B. „An"). Unterstützt alle Binding-Arten: `literal`, `state`, `store`, `query`, `routeParam`, `msg`, `flow`, `global`, `jsonata`, `env`. |
| `labelOff` | „Label Off" | typedInput (alle Binding-Arten) | optional | Text, der im Ausgeschaltet-Zustand neben oder innerhalb des Switches angezeigt wird (z. B. „Aus"). Unterstützt alle Binding-Arten: `literal`, `state`, `store`, `query`, `routeParam`, `msg`, `flow`, `global`, `jsonata`, `env`. |
