...
| `writeTrigger` | „Write Trigger" | SelectBox (`none` / `change` / `submit`) | optional | Default `submit`; bei einer Checkbox ohne Wirkung — es wird effektiv immer bei `change` geschrieben. Ausnahme `none` (P206): **kein** automatischer Write-Back (auch nicht bei `change`) — der `change`-Output feuert weiter, der Flow-Autor verdrahtet die Persistenz selbst; `writeTo` darf bei `none` leer sein. |
