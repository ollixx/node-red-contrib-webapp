...
| `writeTrigger` | „Write Trigger" | SelectBox (`none` / `change` / `submit`) | optional | Default `submit` (= Enter/Blur). `change` schreibt bei jeder Auswahl. `none` (P206) = **kein** automatischer Write-Back — der Flow-Autor verdrahtet die Persistenz selbst; die `change`/`submit`-Output-Events feuern weiter, `writeTo` darf bei `none` leer sein. |
