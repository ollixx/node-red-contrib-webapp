...
| `fallbackSrc` | „Fallback URL" | typedInput (Binding) | optional | URL, die angezeigt wird, wenn das Laden der `src` fehlschlägt. Bindbar über alle Standard-Binding-Arten: `literal` (statische URL als ein Fall), `state`, `store`, `query`, `routeParam`, `msg`, `flow`, `global`, `jsonata`, `env`. |
