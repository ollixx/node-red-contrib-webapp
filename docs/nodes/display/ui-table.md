...
| `rows` | „Rows" | typedInput (Binding) | **ja** | Array der Tabellenzeilen. Bindbar über alle Standard-Binding-Arten: `literal`, `state`, `query`, `routeParam`, `store`, `msg`/`flow`/`global`/`jsonata`/`env`. Jedes Zeilenobjekt sollte die in `columns` deklarierten Keys enthalten; fehlende Keys werden als leerer String gerendert. Binding-Arten: [stores.md](../concepts/stores.md). |
