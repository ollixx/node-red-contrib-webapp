...
| Feld | Label | Editor-Typ | Pflicht | Beschreibung |
|---|---|---|---|---|
| `layout` | „Child Layout" | SelectBox (Layout-Preset) | **ja** | Preset, nach dem die direkten Kinder dieses Containers angeordnet werden. Auswahl aus `vertical`, `horizontal`, `app`, `grid`, `absolute`. Default: `vertical`. Bestimmt die verfügbaren Kinder-Slots und deren Platzierungs-Felder. Die SelectBox schreibt den gewählten Preset in das Feld `layout`. Details: [layout.md](../concepts/layout.md). |
